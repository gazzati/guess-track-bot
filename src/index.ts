...
class Main extends Base {
  ...
  private async start(msg: Message) {
    this.send(
      msg.chat.id,
      "Привет, давай сыграем в игру \nТы присылаешь мне имя исполнителя, а я тебе фрагмент из его песни \nТебе нужно угадать название песни 🤙",
      this.startKeyboardButtons
    )

    this.storage.removeChat(msg.chat.id)
  ...
  private chooseArtist(chat: Chat) {
    this.storage.removeChat(chat.id)
  ...
  private async message(msg: Message) {
    ...
    const chat = await this.storage.getChat(msg.chat.id)
  ...
  private async sendLyric(user: User, chat: Chat, artist: string): Promise<void> {
    ...
    const lyric = await this.getTrackLyric(randomTrack.track_id)
    if (!lyric) return this.error(chat.id, user, "Текст песни не найден 😭")

    const lyricFragment = this.getLyricFragment(lyric)
